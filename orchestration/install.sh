...
if ! dispatcher_health=$(bun "$ROOT/orchestration/dispatcher.ts" health); then
...
' <<<"$dispatcher_health"; then
  printf 'Fleet dispatcher returned an invalid health report: %s\n' "$dispatcher_health" >&2
  exit 1
fi
