...
for command in bun herdr omp bwrap sqlite3 git systemctl apparmor_parser; do
...
for unit in unitb-omp-auth-broker.service unitb-herdr-supervisor.service unitb-fleet-dispatcher.service; do
  sed "s|@REPO_ROOT@|$ROOT|g" "$ROOT/orchestration/systemd/$unit" > "$USER_SYSTEMD_DIR/$unit"
done

systemctl --user daemon-reload
systemctl --user enable --now unitb-omp-auth-broker.service unitb-herdr-supervisor.service unitb-fleet-dispatcher.service
