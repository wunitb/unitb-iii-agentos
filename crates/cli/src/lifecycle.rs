...
#[cfg(target_os = "linux")]
fn process_is_gone(process: &OwnedProcess) -> bool {
    match process_identity(process.pid) {
        Ok(identity) => identity.zombie,
        Err(_) => true,
    }
}
...
#[cfg(target_os = "linux")]
fn terminate_group(process: &OwnedProcess) -> Result<()> {
    // Bind authority to the directly spawned leader immediately before the two
    // group-directed syscalls. SIGKILL follows SIGTERM without a pathname/name
    // lookup or a second pid-resolution window, so every descendant in the
    // owned process group receives a terminal signal even when the leader exits
    // on SIGTERM first.
    ...
    let term = unsafe { kill(-group, 15) };
    if term != 0 {
        return Err(std::io::Error::last_os_error())
            .context(format!("Cannot terminate owned process group {group}"));
    }
    // ESRCH here only means SIGTERM already emptied the exact group. Any other
    // member that survived receives SIGKILL.
    let _ = unsafe { kill(-group, 9) };
    Ok(())
}

pub(crate) fn stop_owned(agentos_home: &Path) -> Result<StopOutcome> {
    ensure_supported()?;
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        while Instant::now() < deadline
            && record
                .processes
                .iter()
                .any(|process| !process_is_gone(process))
        {
        ...
        if let Some(process) = record
            .processes
            .iter()
            .find(|process| !process_is_gone(process))
        {
            anyhow::bail!(
...
mod tests {
    ...
    fn no_record_never_signals_an_unrelated_process() {
        ...
        assert_eq!(stop_owned(&root).unwrap(), StopOutcome::NothingRecorded);
    ...
    fn persisted_identity_stops_only_the_owned_group() {
        ...
        assert_eq!(stop_owned(&root).unwrap(), StopOutcome::Stopped(1));
        let _ = child.wait();
    ...
    fn tampered_start_token_refuses_without_signalling() {
        ...
        let error = stop_owned(&root).unwrap_err().to_string();
